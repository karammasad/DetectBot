...
def reply():
    ...
    for tweet in reversed(tweets):
        if tweet.in_reply_to_status_id:
            og_tweet = api.get_status(tweet.in_reply_to_status_id)
            if 'media' in og_tweet.entities:
                ...
                if (colorDetection.percent_grey > 0.63):
                    tweet_message = ' The image attached to the original tweet is fabricated or has been manipulated with a certanity of ' + str(colorDetection.percentCertanity) + '% ❌'
                elif (colorDetection.percent_grey <= 0.63):
                    tweet_message = ' The image attached to the original tweet is real (minimal manipulation) with a certanity of ' + str(colorDetection.percentCertanity) + '% ✅'
                print(tweet.id)
                ...
                api.update_status('@' + tweet.user.screen_name + "The tweet doesn't include an image to be analyzed", tweet.id)
